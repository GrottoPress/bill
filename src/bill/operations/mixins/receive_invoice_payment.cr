...
module Bill::ReceiveInvoicePayment
  macro included
    ...
    private def validate_amount_eq_invoice
      invoice.try do |invoice|
        amount.value.try do |value|
          return if invoice.net_amount == value
    ...
    private def mark_invoice_as_paid(receipt : Bill::Receipt)
      return unless ReceiptStatus.now_finalized?(status)

      invoice.try do |invoice|
        return unless invoice.net_amount == receipt.amount
